...
"""NewType: a generic numerical type"""

Vector=NewType("Vector", List[Numeric])
"""NewType: A generic vector type."""
...
def summary(data: SciData, *rest, angular=False, radians=True)->OrderedDict:
    ...
    if angular:
        kwargs = {"high": 2 * np.pi,
                  "low": 0} if radians else {"high": 360, "low": 0}
...
def textSummary(
        data: SciData, angular: bool=False, radians: bool=True) ->Text:
    """
    ...
    if angular:
        ...
        strings.append("=" * 20)
        strings += [f"{key+':':<10.12s} {value:>9.3f}" for key,
                    value in summary(data, angular=True, radians=radians).items()]
        strings.append("=" * 20)
    else:
        strings.append("{0:^20.20s}".format("Data Summary"))
        strings.append("=" * 20)
        strings += [f"{key+':':<10.12s} {value:>9.3f}" for key,
                    value in summary(data).items()]
        strings.append("=" * 20)
    return "\n".join(strings)


def boxcar(data: SciData, carlength: int=5,
           func: Callable[[SciData], Numeric]=np.mean) -> Tuple[Vector,Vector]:
    """
